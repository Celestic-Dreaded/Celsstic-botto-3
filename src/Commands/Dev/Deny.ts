...
export default class command extends BaseCommand {
    override execute = async ({ sender, reply }: Message, { context }: IArgs): Promise<void> => {
        if (!context)
            return void reply(
                'ðŸŸ¥ *Provide the command that you want it to be denied globally with the reason for denying it*'
            )
        const terms = context.trim().split('|')
        const cmd = terms[0].toLowerCase().trim()
        if (!terms[1])
            return void reply(
                `ðŸŸ¥ *Provide the reason for denying it*. Example - *${this.client.config.prefix}deny hi | Under maintenance*`
            )
        ...
        if (index >= 0)
            return void reply(
                `ðŸŸ¨ *${this.client.utils.capitalize(cmd)}* is already denied globally by *${
                    commands[index].deniedBy
                }* in *${commands[index].time} (GMT)*. ðŸŽˆ Reason: *${commands[index].reason}*`
            )
        const reason = terms[1].trim()
        await this.client.DB.denyCommand(command.name, reason, sender.username)
        return void reply(`ðŸŽ *${this.client.utils.capitalize(cmd)}* is denied globally now. ðŸŽˆ Reason: *${reason}*`)
    }
}
